---
layout: ../../layouts/BaseLayout.astro
title: Resume
description: Career summary and resume for John MacDonald.
---
# John MacDonald

[linkedin.com/in/jmacego](https://www.linkedin.com/in/jmacego) | Los Angeles, CA

Download the current resume: [john-macdonald-resume.pdf](/assets/files/john-macdonald-resume.pdf)

---

## Technology Leader & Serial Entrepreneur

Founder and operator of multiple successful ventures spanning technology, media, aviation, and retail. Award-winning engineering leader with deep expertise in AI/ML, network architecture, and operational transformation. Currently building and advising companies while selectively taking on leadership and consulting engagements.

Recipient, **Disney Inventor Award (2021)** for developing algorithms and code that apply machine learning to networking.

---

## Selected Career Highlights

- Founded and operated 5+ companies across technology, aviation, photography, and retail.
- Pioneered use of linear regression and machine learning to accurately predict file transfer times at Disney, earning U.S. Patent recognition.
- Achieved **97% reduction in outages** by building Disney Studios' first 24/7 global network operations center.
- Slashed **$4M in annual expenses** while reducing outages by 80% through strategic technology decisions.

---

## Current Ventures

### Equivalent Smart Graphics | Remote
**Fractional CTO**  
*February 2026 – Present*

- Providing strategic technology leadership for AI-powered accessible SVG solutions company.
- Guiding infrastructure, API architecture, and enterprise scalability for Smart SVG platform.

---

## Corporate Leadership Experience

### The Walt Disney Studios | Burbank, CA
**Senior Manager, Network Engineering Operations**  
*January 2019 – January 2026*

- Promoted 3 times in 6 years; progressed from Senior Engineer to Senior Manager leading team of 19.
- Directed global network operations across Marvel, Twentieth Century, Disney Live Action, and Searchlight.
- Managed budgets up to $14.5M; delivered 3 VxLAN/EVPN datacenters for high-performance compute.
- Advanced AI/ML initiatives to drive automation and predictive network management.
- Key technical lead during pandemic recovery, restoring collapsed network environment.

---

### Sohonet Ltd. | Marina Del Rey, CA
**Network Engineer**  
*2017 – 2019*

- Designed MPLS and BGP solutions for global media clients including Apple, Google, and Amazon.
- Deployed DWDM and MPLS infrastructure spanning 4 continents and dozens of POPs.

---

## Founded & Operated Companies

### Sheep's Clothing | Kennewick, WA
**Owner**  
*2010 – 2015*

- Built specialty retail business from startup to successful exit.
- Grew revenue **10x** through product curation, customer experience, and local marketing.
- Managed all aspects: inventory, staffing, finance, vendor relationships, and marketing.
- Successfully sold business to new ownership.

---

### Avitus Aviation Corp | Gig Harbor, WA
**President**  
*2001 – 2010*

- Founded and operated aviation leasing and services company.
- Managed aircraft fleet, pilot operations, and client relationships.
- Built expertise in risk management, safety culture, regulatory compliance, and operational resilience.
- Developed business acumen balancing capital-intensive operations with service delivery.

---

### Hyak Photography / Hyakdev.com LLC | Snoqualmie Pass, WA
**Owner**  
*2005 – 2010*

- Founded photography and web development business serving Pacific Northwest clients.
- Delivered commercial photography, web design, and custom software solutions.
- Built client base through reputation and referrals in competitive market.

---

### Earlier Ventures

**Website Management Systems** | Las Vegas, NV (2016–2017)
- Came out of retirement to relaunch technology career through software development and DevOps.

**White Dog Media, LLC** | Las Vegas, NV (2001–2003)
- Partner in digital media venture during early web era.

**5GuysTech.com, LLC** | Seattle, WA (1997–2001)
- Senior Partner in managed services and web hosting company during dot-com boom.

**PowerSurge Technologies / FastServers.net** | Cedar Rapids, IA (1995–1996)
- Co-founded one of the early dedicated server hosting companies.
- Built infrastructure and customer base before successful exit.

---

## Summary of Qualifications

- Serial entrepreneur with **25+ years** founding and operating technology, aviation, retail, and media companies.
- Award-winning technology leader with expertise in AI/ML, network architecture, and operational transformation.
- **U.S. Patent 12,360,789** inventor for machine learning methods in cloud networking.
- Proven ability to build teams, manage budgets, and deliver measurable results in both startups and enterprises.
- Available for fractional leadership, advisory, and consulting engagements.

---

## Awards & Certifications

- **U.S. Patent No. 12,360,789 (2025):** Machine learning methods for optimized cloud networking file transfers.
- **Disney Inventor Award (2021):** Applied machine learning algorithms to networking.
- Previously held certifications: Cisco (CCDP, CCNP), Juniper (JNCDA), Microsoft (MCP+i).

---

## Education

- **AAS, Biology** – Columbia Basin College | Pasco, WA

---
